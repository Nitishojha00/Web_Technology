...
p.append(`Q1.  ${Qlist[idx]["question"]}`);
div.append(p);
for(let j=0;j<4;j++)
{
    ...
    i.type = "radio";
    i.name = "Q1"; // abhi bs Q1 kuki ek element krrha
    i.value = `${Qlist[idx]["options"][j]}`;
    l.append(i);
    const optName = document.createTextNode(`${Qlist[idx]["options"][j]}`);
    l.append(optName);
...
form.addEventListener('submit',(e)=>{  
    ...
    console.log(data.get('Q1')); //  nyc new learning
})
